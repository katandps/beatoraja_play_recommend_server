use anyhow::Result;
use beatoraja_play_recommend::google_id::GoogleId;
use beatoraja_play_recommend::{Account, MySQLClient};
...
pub fn get_client() -> RedisResult<Connection> {
    let client = redis::Client::open(beatoraja_play_recommend::config().redis_url())?;
